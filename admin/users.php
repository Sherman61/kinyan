...
?>
<section class="dashboard"><div class="page-title"><h1>Users</h1><p>Manage roles, trust levels, and account activity.</p></div><div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Trust</th><th>Cars</th><th>Wanted</th><th>Action</th></tr></thead><tbody>
<?php foreach ($users as $u): ?><tr><td><strong><?= e($u['name']) ?></strong><br><?= e($u['email']) ?></td><td><?= e($u['role']) ?></td><td><?= e(trust_label((int)($u['trust_level'] ?? 1))) ?></td><td><?= (int)$u['cars'] ?></td><td><?= (int)$u['wanted'] ?></td><td><form method="post" class="inline-controls"><?= csrf_field() ?><input type="hidden" name="id" value="<?= (int)$u['id'] ?>"><select name="role"><option value="user" <?= selected($u['role'], 'user') ?>>user</option><option value="admin" <?= selected($u['role'], 'admin') ?>>admin</option></select><select name="trust_level"><option value="1" <?= selected($u['trust_level'] ?? 1, 1) ?>>Trust 1</option><option value="2" <?= selected($u['trust_level'] ?? 1, 2) ?>>Trust 2</option><option value="3" <?= selected($u['trust_level'] ?? 1, 3) ?>>Trust 3</option></select><button data-confirm="Save this user's role and trust level?"><span aria-hidden="true">✓</span>Save</button></form></td></tr><?php endforeach; ?>
