...
$posts = db()->query('SELECT w.*, u.email user_email FROM wanted_posts w JOIN users u ON u.id = w.user_id ORDER BY w.created_at DESC LIMIT 200')->fetchAll();
render_header('Admin Wanted Posts', 'Manage Kinyan wanted posts.');
?>
<section class="dashboard"><div class="page-title"><h1>Wanted posts</h1><p>Approve, reject, edit, or delete buyer requests.</p></div><nav class="admin-tabs"><a href="index.php">Admin</a><a href="listings.php">Listings</a></nav><div class="table-wrap"><table><thead><tr><th>Post</th><th>Status</th><th>User</th><th>Actions</th></tr></thead><tbody>
