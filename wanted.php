...
$countStmt = db()->prepare('SELECT COUNT(*) FROM wanted_posts WHERE ' . implode(' AND ', $where));
$countStmt->execute($params);
$pagination = pagination_state((int)$countStmt->fetchColumn());
$stmt = db()->prepare('SELECT * FROM wanted_posts WHERE ' . implode(' AND ', $where) . " ORDER BY $sort LIMIT {$pagination['per_page']} OFFSET {$pagination['offset']}");
$stmt->execute($params);
$posts = $stmt->fetchAll();

$renderResults = static function () use ($posts, $pagination): void {
    if ($posts) {
        echo '<div class="stack">';
        foreach ($posts as $post) render_wanted_card($post);
        echo '</div>';
    } else {
        echo '<div class="empty-state"><h3>No wanted posts match</h3><p>Try a broader search or post what you are looking for.</p><a class="button ghost" href="wanted.php">Clear filters</a></div>';
    }
...
<section class="browse-layout filters-collapsed" data-browse-layout>
    <aside class="filters" id="wanted-filters" data-filters aria-label="Wanted car filters">
        <button class="filter-close" type="button" data-filter-toggle aria-controls="wanted-filters" aria-expanded="false">Close filters</button>
    ...
    <div class="browse-main">
        <div class="browse-toolbar"><button class="button secondary filter-toggle-button" type="button" data-filter-toggle aria-controls="wanted-filters" aria-expanded="false">Filters</button><span data-results-count><?= (int)$pagination['total'] ?> wanted posts</span><form method="get" data-sort-form><?php foreach ($_GET as $k => $v): if ($k !== 'sort' && $k !== 'page'): ?><input type="hidden" name="<?= e($k) ?>" value="<?= e($v) ?>"><?php endif; endforeach; ?><select name="sort" onchange="this.form.requestSubmit()"><option value="newest">Newest</option><option value="oldest" <?= selected($_GET['sort'] ?? '', 'oldest') ?>>Oldest</option><option value="budget_high" <?= selected($_GET['sort'] ?? '', 'budget_high') ?>>Highest budget</option><option value="budget_low" <?= selected($_GET['sort'] ?? '', 'budget_low') ?>>Lowest budget</option></select></form></div>
