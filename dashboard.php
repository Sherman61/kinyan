...
<section class="dashboard">
    ...
    <section class="details-card">
        ...
        <div class="table-wrap"><table><thead><tr><th>Listing</th><th>Status</th><th>Views</th><th>Contact clicks</th><th>Actions</th></tr></thead><tbody>
        <?php foreach ($carRows as $car): $stats = $carStats[(int)$car['id']] ?? ['calls'=>0,'texts'=>0,'emails'=>0,'copy_links'=>0,'shares'=>0,'total'=>0]; ?><tr><td><strong><?= e($car['title']) ?></strong><br><span><?= e($car['year'] . ' ' . $car['make'] . ' ' . $car['model']) ?></span></td><td><span class="badge"><?= e($car['status']) ?></span></td><td><?= (int)$car['views'] ?></td><td><div class="stats-mini"><strong><?= (int)$stats['total'] ?> total</strong><span>Call <?= (int)$stats['calls'] ?> · Text <?= (int)$stats['texts'] ?> · Email <?= (int)$stats['emails'] ?></span><span>Copy <?= (int)$stats['copy_links'] ?> · Share <?= (int)$stats['shares'] ?></span></div></td><td class="table-actions"><a class="icon-action" data-confirm="Open editor for this listing?" href="post-car.php?id=<?= (int)$car['id'] ?>"><span aria-hidden="true">✎</span>Edit</a><a class="icon-action" href="listing.php?id=<?= (int)$car['id'] ?>"><span aria-hidden="true">↗</span>View</a><form method="post"><?= csrf_field() ?><input type="hidden" name="type" value="car"><input type="hidden" name="id" value="<?= (int)$car['id'] ?>"><?php foreach (status_options_for_user($car, 'car_listings') as $statusValue => $statusLabel): ?><button name="action" value="<?= e($statusValue) ?>" data-confirm="Set this listing to <?= e(strtolower($statusLabel)) ?>?"><span aria-hidden="true"><?= $statusValue === 'active' ? '✓' : ($statusValue === 'sold' ? '$' : '−') ?></span><?= e($statusLabel) ?></button><?php endforeach; ?><button class="danger-link" name="action" value="delete" data-confirm="Delete this car listing permanently?"><span aria-hidden="true">×</span>Delete</button></form></td></tr><?php endforeach; ?>
        </tbody></table></div>
    </section>
    <section class="details-card">
